...
$sql = "SELECT user.user_name, user.user_image, course_review.review_rate, course_review.review_content, course_review.review_reg_date FROM course_review"
		." INNER JOIN user ON course_review.user_idx = user.user_idx WHERE course_review.course_idx = $_GET[course_idx] ORDER BY course_review.review_reg_date DESC LIMIT $_GET[start] , 3";
