...
$sql = "SELECT user.user_name, user.user_image, guide_review.review_rate, guide_review.review_content, guide_review.review_reg_date FROM user"
		." INNER JOIN guide_review ON guide_review.user_idx = user.user_idx WHERE guide_review.guide_idx = $_GET[guide_idx] ORDER BY guide_review.review_reg_date DESC LIMIT $_GET[start] , 3";
